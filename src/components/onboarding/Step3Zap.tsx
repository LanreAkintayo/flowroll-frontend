...
export function Step3Zap({ isComplete, isUnlocked, evmAddress }: Step3ZapProps) {
    ...
    const { balances, currentAllowance } = useOnboardingQueries(evmAddress);
    ...
    const initBalance = balances.init;
    const amountNum = Number(zapInput);
    const isValidInput = amountNum > 0 && amountNum <= initBalance;
    const needsApproval = amountNum > currentAllowance;
            ...
            <div className="bg-slate-50 dark:bg-[#0f0f0f] rounded-2xl p-2 border border-slate-100 dark:border-slate-800/80">
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <input
                        ...
                        className="flex-1 w-full h-12 bg-transparent border-none outline-none text-lg font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 px-4"
                    ...
                    <Button
                        ...
                        className={`h-12 px-8 rounded-xl font-bold cursor-pointer transition-colors duration-300 border-none shrink-0 ${
