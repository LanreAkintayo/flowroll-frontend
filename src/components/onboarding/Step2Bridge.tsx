...
export function Step2Bridge({ isComplete, isUnlocked, evmAddress }: Step2BridgeProps) {
    ...
    return (
        ...
        >
                ...
                <Button
                    ...
                    className={`w-full sm:w-auto shrink-0 h-12 px-8 rounded-xl font-bold cursor-pointer transition-colors duration-300 border-none ${
                        !isUnlocked
                            ...
                            : isComplete 
                                ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 opacity-80' 
            ...
            {!isComplete && <div className="pt-6 border-t border-slate-100 dark:border-slate-800/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <Button
                    ...
                    variant="outline"
                    disabled={!isUnlocked}
                    className="w-full sm:w-auto shrink-0 h-10 px-4 rounded-lg font-semibold text-xs cursor-pointer border-amber-200 dark:border-amber-900/50 text-amber-700 dark:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-500/10 bg-transparent transition-colors"
                >
                    Open Bridge
