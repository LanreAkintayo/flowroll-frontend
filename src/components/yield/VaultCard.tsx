...
export function VaultCard({ cycleId, poolIndex, poolEntry }: VaultCardProps) {
  ...
  const { data: details, isLoading: loadingDetails } = usePoolDetails(
    poolEntry.pool,
  );
  const { data: allocation, isLoading: loadingAllocation } = usePoolData(
    cycleId,
    poolIndex,
    poolEntry.pool,
  );
  ...
  const apy = details?.apyBps
    ? (Number(details.apyBps) / 100).toFixed(1)
    : "0.0";
  const balanceUsdc = allocation?.valueUsdc
    ? formatMoney(allocation.valueUsdc, 6)
    : "0.00";
  const sharesAmount = allocation?.shares
    ? formatMoney(allocation.shares, 6)
    : "0.00";

  const isStable = details?.isStablePair ?? poolEntry.isStablePair;
  const Icon = isStable ? ShieldCheck : Activity;

  const iconBg = isStable
    ? "bg-emerald-50 dark:bg-emerald-500/10"
    : "bg-blue-50 dark:bg-blue-500/10";
  const iconColor = isStable
    ? "text-emerald-600 dark:text-emerald-400"
    : "text-blue-600 dark:text-blue-400";
  const iconBorder = isStable
    ? "border-emerald-200 dark:border-emerald-500/20"
    : "border-blue-200 dark:border-blue-500/20";

  // Invalidate pool data queries reactively when the cycle parameters shift
  useMemo(() => {
  ...
  }, [cycleData?.idleBalance, cycleData?.totalDeposited, address, cycleId, poolIndex, queryClient]);
    ...
    >
      <div className="p-4 sm:p-6 pb-6 sm:pb-8 flex-1">
        <div className="flex justify-between items-start mb-6 sm:mb-8 gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <div
                className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 border ${iconBorder} ${iconBg}`}
              >
        ...
        <div className="flex flex-col min-w-0">
          ...
          <div className="flex items-baseline gap-1.5 w-full">
            ...
            <span className="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-600 shrink-0">
              USDC
            </span>
          </div>
        </div>
      ...
      <div className="bg-slate-50/50 dark:bg-[#0f0f0f] border-t border-slate-200 dark:border-slate-800/80 p-4 sm:p-5">
        <div className="flex justify-between items-center gap-2">
          <div className="min-w-0">
            <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-0.5 sm:mb-1">
              Yield Rate
            </p>
            <div className="flex items-center gap-1 sm:gap-1.5">
              ...
              <p
                className={`text-xs sm:text-sm font-bold tabular-nums truncate ${apy === "0.0" ? "text-slate-500 dark:text-slate-600" : "text-emerald-600 dark:text-emerald-400"}`}
              >
...
//   // I need a useEffect here or something better here. which will help in updating the pool data whenever there is a change in the cycleData idlebalance or totaldeposited. This is what usePayrollCycle returns;
//   /**
...
//    }
//      So, what i want now is that if the idleBalance or toaldeposited changes, then I want to somehow refetch the pool data. This is how usePoolData looks like;
