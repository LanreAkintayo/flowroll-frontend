                      ...
                      {(['employer', 'employee'] as const).map((r) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            {r === 'employer' ? <Building2 className={`w-4 h-4 ${role === r ? 'text-teal-500' : 'text-slate-400'}`} /> : <User className={`w-4 h-4 ${role === r ? 'text-teal-500' : 'text-slate-400'}`} />}
                            <span className={`capitalize font-medium ${role === r ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}>
                              {r}
                            </span>
                          </div>
                          {role === r && <Check className="w-4 h-4 text-teal-500" />}
                        </button>
                      ))}
                    </motion.div>
                  )}
          ...
          >
            <div className="px-4 py-8 flex flex-col gap-6 min-h-full">
              ...
              <section>
                ...
                <div className="flex flex-col gap-1 bg-slate-100/70 dark:bg-[#0a0c10] border border-slate-200 dark:border-slate-800 rounded-[2rem] p-2">
              ...
              <section className="mt-auto pt-6">
                <div className="bg-slate-100/70 dark:bg-[#0a0c10] border border-slate-200 dark:border-slate-800 rounded-[2rem] p-4 flex flex-col gap-4">
                  {isApp && (
                    <>
                      <div className="flex flex-col gap-4 px-2">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-bold text-slate-900 dark:text-white">Network</p>
                            <p className="text-xs font-medium text-slate-500">Active Chain</p>
                          </div>
                          <NetworkSwitcher />
                        </div>
...
//   // Handle click-outside for workspace switcher
//   useEffect(() => {
//     const handleClickOutside = (event: MouseEvent) => {
//       if (dropdownRef.current && !dropdownRef.current.contains(event.target as Node)) {
//         setIsWorkspaceOpen(false);
...
//   // Sync mobile menu state with route changes
//   useEffect(() => {
//     setIsMobileMenuOpen(false);
//   }, [pathname]);
