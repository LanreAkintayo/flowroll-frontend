  ...
  return (
    <div className={`mx-auto space-y-12 ${className}`}>
      <div className="space-y-10">
        ...
        {/* Section Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-4 border-t border-slate-200 dark:border-slate-800">
          ...
          <Button
            ...
            className="bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-200 text-white dark:text-slate-900 rounded-md px-6 py-3 h-auto font-bold transition-all hover:shadow-xl shadow-slate-900/20 dark:shadow-white/5 group"
          >
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className="h-64 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] animate-pulse"
                />
              ))}
            </div>
          ) : !employerGroups || employerGroups.length === 0 ? (
            // Empty State
            <div className="bg-white dark:bg-[#0a0c10] border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[3rem] p-20 text-center flex flex-col items-center justify-center">
            ...
            // Active Groups Grid
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {/* Safely copy the array before reversing to avoid mutating React Query's cache */}
